...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'BiFusion', 'RepBlock', 'C3_EMBC', 'C2f_EMBC', 'SPPF_LSKA', 'C3_DAttention', 'C2f_DAttention', 'C3_Parc', 'C2f_Parc', 'C3_DWR', 'C2f_DWR','C2f_Faster_SiMAM']
...
######################################## C2f-Faster-EMA end ########################################
######################################## C2f-Faster-SiMAM bigin ########################################
class Faster_Block_SiMAM(nn.Module):
    def __init__(self,
                 inc,
                 dim,
                 n_div=4,
...
class C3_Faster_SiMAM(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Faster_Block_SiMAM(c_, c_) for _ in range(n)))


class C2f_Faster_SiMAM(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Faster_Block_SiMAM(self.c, self.c) for _ in range(n))
####################################### C2f-Faster-SiMAM end #########################################
######################################## C2f-DDB begin ########################################
